...
exports.createReport = (report) => {
    return new Promise((resolve, reject) => {
        ...
        const today = dayjs().format('MMMM D, YYYY h:mm A');
          ...
          const sql = 'INSERT INTO Reports(Mood, Smoked, Feelings, Time) VALUES(?,?,?,?)';
          db.run(sql, [report.mood, report.smoked, report.feelings, today], function (err) {
            if (err) {
              reject(err);
            }
...
exports.updateReport = async (id, report) => {
    ...
    return new Promise(async (resolve, reject) => {
      const sql = 'UPDATE Reports SET Mood=?, Smoked=?, Feelings=? WHERE id=?';
       ...
       db.run(sql, [report.mood, report.smoked, report.feelings, id], function (err) {
        if (err) {
          reject(err);
        }
        if (this.changes !== 1) {
